...
BounceBall::BounceBall(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::BounceBall),p1(600,0),p3(952,0),p1_h(300),p3_h(400),rect_x_vel(-5),p2(150,150),score("0"),intScore(0),gameOver(false),bestScore("0"),intBestScore(0),pl(0,540),pl1(593,540),skin("0"),b1(p2),i(0),a(1),
...
void BounceBall::checkCrash()
{
   ...
   {
        if(b1.getCenter().y()<-50+p1_h || b1.getCenter().y()>= p1_h+24)
        {
           gameOver = true;
        }
   }
       ...
       {
            if( b1.getCenter().y()<-50+p3_h || b1.getCenter().y()>= p3_h+24)
            {
                gameOver = true;
            }
       }
...
{
            if(p1.x() >= 109 && p1.x()<=117 || p3.x()>= 109 && p3.x()<=117)
            {
                intScore++;
            }
}
